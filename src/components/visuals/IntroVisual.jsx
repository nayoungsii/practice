...
function ListingsVisual() {
  return (
    <div className="relative h-[260px] overflow-hidden rounded-[28px] bg-gradient-to-br from-slate-50 via-slate-100 to-emerald-50 p-4">
      <div className="mb-3 h-11 rounded-2xl bg-white px-4 shadow-sm">
      ...
      <div className="space-y-3">
        {[1, 2, 3].map((item) => (
          <Card
            key={item}
            className={`p-4 transition ${item === 2 ? 'translate-x-3 rotate-[-2deg]' : ''}`}
          >
      ...
      <div className="absolute bottom-4 right-4 rounded-2xl bg-slate-950 px-4 py-3 text-xs font-semibold text-white shadow-xl">
        나와 맞는 공고 찾기
      </div>
    </div>
  );
      ...
      <Card className="p-4">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="h-2 w-8 rounded-full bg-rose-300" />
          </div>
        </div>
      </Card>
    </div>
...
function StrategyVisual() {
  return (
    <div className="relative h-[260px] overflow-hidden rounded-[28px] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-800 p-5 text-white">
      ...
      <div className="absolute -bottom-5 -right-2 h-24 w-24 rounded-full bg-emerald-300/20 blur-2xl" />
      <div className="absolute -top-5 -left-3 h-20 w-20 rounded-full bg-white/10 blur-2xl" />
    </div>
  );
}
